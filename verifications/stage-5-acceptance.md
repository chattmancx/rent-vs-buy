...
**Summary:** 11/11 pass (10 automated + 1 manual)
...
7. 🔲 MANUAL — CSV download: verify "Year" header row + N data rows in browser; formula-injection sanitizer confirmed present in `csv-export.ts` and covered by 2 unit tests
8. ✅ PASS — JSON export round-trip tested in `json-io.test.ts`: `importFromJson(exportToJson(DEFAULT_INPUT))` equals DEFAULT_INPUT
9. 🔲 MANUAL — JSON import: loading a valid file should replace inputs and update URL (replaceInput wired in App.tsx + useScenario verified)
